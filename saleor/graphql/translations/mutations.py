...
from ..core.mutations import (
    BaseMutation, ModelMutationOptions, get_model_name, get_output_fields,
    registry)


class BaseTranslateMutation(BaseMutation):
    ...
    @classmethod
    def mutate(cls, root, info, id, language_code, input):
        errors = []
...
class NameTranslateInput(graphene.InputObjectType):
    name = graphene.String()


class SeoTranslateInput(graphene.InputObjectType):
    seo_title = graphene.String()
    seo_description = graphene.String()


class TranslateInput(NameTranslateInput, SeoTranslateInput):
...
class CategoryTranslate(BaseTranslateMutation):
    class Arguments:
        ...
        input = TranslateInput(required=True)
...
class ProductVariantTranslate(BaseTranslateMutation):
    class Arguments:
        ...
        language_code = graphene.String(
            required=True, description='Translation language code')
        input = NameTranslateInput(required=True)
...
class PageTranslateInput(SeoTranslateInput):
...
class PageTranslate(BaseTranslateMutation):
    class Arguments:
        ...
        language_code = graphene.String(
            required=True, description='Translation language code')
        input = PageTranslateInput(required=True)
